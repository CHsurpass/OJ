...
import top.qoj.pojo.vo.ConfigVO;
...
public class StartupRunner implements CommandLineRunner {
    ...
    private void upsertHOJLanguage(String... languageList) {
        ...
        for (String language : languageList) {
            ...
            if (count == 0) {
                Language newLanguage = buildQOJLanguage(language);
                boolean isOk = languageEntityService.save(newLanguage);
                if (!isOk) {
                    log.error("[Init System Config] [QOJ] Failed to add new language [{}]! Please check whether the language table corresponding to the database has the language!", language);
                }
            }
        }
    }
    ...
    private Language buildQOJLanguage(String lang) {
