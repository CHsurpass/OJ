...
public class JudgeValidator {
    ...
    public void validateSubmissionInfo(SubmitJudgeDTO submitJudgeDto) throws StatusFailException, AccessException {
        ...
        if (submitJudgeDto.getGid() != null) { // 团队内的提交
            accessValidator.validateAccess(QOJAccessEnum.GROUP_JUDGE);
        } else if (submitJudgeDto.getCid() != null && submitJudgeDto.getCid() != 0) {
            accessValidator.validateAccess(QOJAccessEnum.CONTEST_JUDGE);
        } else {
            accessValidator.validateAccess(QOJAccessEnum.PUBLIC_JUDGE);
        }
        ...
        if (submitJudgeDto.getCode().length() < 20
            ...
            throw new StatusFailException("提交的代码是无效的，代码字符长度请不要低于20！");
        }
    ...
    public void validateTestJudgeInfo(TestJudgeDTO testJudgeDto) throws StatusFailException, AccessException {
        ...
        // Remote Judge的编程语言需要转换成QOJ的编程语言才能进行自测
        ...
        if (testJudgeDto.getCode().length() < 20
